...
const CGFloat kGHRevealSidebarWidth = 260;
...
- (void)setSidebarViewController:(UIViewController *)svc {
    if (sidebarViewController == nil) {
        ...
        _sidebarView.frame = CGRectOffset(_sidebarView.frame, CGRectGetWidth([[UIScreen mainScreen] bounds]) - kGHRevealSidebarWidth, 0);
...
- (void)dragContentView:(UIPanGestureRecognizer *)panGesture {
    ...
    if (panGesture.state == UIGestureRecognizerStateChanged) {
        ...
        if (sidebarShowing) {
            BOOL show = (CGRectGetMaxX(_contentView.frame) > CGRectGetMidX([UIScreen mainScreen].bounds)) ? false : true;
...
- (void)toggleSidebar:(BOOL)show duration:(NSTimeInterval)duration {
    ...
    if (self.delegate) {
        if (self.currentState != show) {
            ...
            if([self.delegate respondsToSelector:@selector(sliderMenuDidComplete:)]) {
                [self.delegate sliderMenuDidComplete:show];
            }
        }
    }
}
